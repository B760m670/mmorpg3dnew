...
##
## ОДИН палец (в любом месте) — поворот. Палец вниз → камера вниз, вверх → вверх;
##   палец вправо → камера налево, влево → камера направо. Поворот следует за
##   пальцем (скорость поворота = скорость пальца). Yaw без предела (360°),
##   pitch ограничен ±89° (без переворота горизонта).
## ДВА пальца — зум + перенос. Сводишь пальцы → приближение, разводишь →
##   отдаление (доллирование вдоль взгляда). Смещение двух пальцев → перенос
##   камеры за пальцами (без реверса).
...
func _rotate_by(rel: Vector2) -> void:
	# палец вправо (rel.x>0) → поворот налево (+yaw); палец вниз (rel.y>0) → вниз (−pitch)
...
func _two_finger() -> void:
	...
	# доллирование вдоль взгляда: свёл пальцы → вперёд (ближе)
