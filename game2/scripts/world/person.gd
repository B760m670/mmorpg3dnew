...
## НАБОР ДВИЖЕНИЙ И СОБСТВЕННАЯ СКОРОСТЬ КАЖДОГО КЛИПА.
##
## Скорость — не украшение отчёта, а то, чем клип ставится в пространство
## смешивания: при своей скорости клип не даёт стопе скользить по построению.
## Числа ИЗМЕРЕНЫ при сборке (studio/export_hero.py печатает их в «[итог]») по
## пройденному за цикл пути, а не назначены.
##
## Почему таблица здесь, а не в файле модели: glTF не везёт наши пометки к
## клипам, а держать рядом второй файл — значит рано или поздно разойтись с
## первым. Здесь же есть САМОПРОВЕРКА: если клипа из таблицы в файле нет или в
## файле есть клип не из таблицы, это печатается вслух.
const СКОРОСТИ := {
	"покой": 0.0,
	"шаг медленно": 0.38,
	"шаг": 1.29,
...
const ПОВОРОТ := "поворот"

var набор: Dictionary = {}      # имя клипа в файле -> его скорость, м/с
var clip_turn := ""
...
func _build_from_file() -> bool:
	...
	if anim != null:
		var есть := anim.get_animation_list()
		for a in есть:
			var короткое := a.get_slice("/", a.get_slice_count("/") - 1)
			if СКОРОСТИ.has(короткое):
				набор[a] = float(СКОРОСТИ[короткое])
				if короткое == "покой":
					clip_idle = a
				elif короткое == "шаг":
					clip = a
			elif короткое == ПОВОРОТ:
				clip_turn = a
			else:
				print("[человек] клип «%s» есть в файле, но не в таблице скоростей" % a)
		for имя in СКОРОСТИ:
			var найден := false
			for a in есть:
				if a.ends_with(имя):
					найден = true
			if not найден:
				print("[человек] НЕТ КЛИПА «%s» — походка будет беднее" % имя)
		# ЗАПАСНОЙ ПУТЬ для старых файлов, где клип один.
		if clip == "" and есть.size() > 0:
			for a in есть:
			...
			if clip == "":
				clip = есть[0]
		# ПЕТЛЯ СТАВИТСЯ ЯВНО. Клипы из glTF приходят с loop_mode = NONE, и
		# смешивание тогда доигрывает клип до конца и замирает: в кадре это
		# читается как «завис на середине шага».
		for a in набор.keys():
			var an := anim.get_animation(a)
			if an != null:
				an.loop_mode = Animation.LOOP_LINEAR
	from_file = true
	print("[человек] тело из файла: костей %d, сеток %d; клипов в наборе %d%s"
		% [skel.get_bone_count(), _count_meshes(root), набор.size(),
			", поворот есть" if clip_turn != "" else ", поворота нет"])
	return true
