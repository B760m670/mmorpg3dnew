...
# протяжённый корпус, две пятигранные башни, боковые каре-крылья, облицовка камнем
clear()
L = 220.0; W = 22.0; Hc = 16.0
box(0, 0, Hc/2, L*0.46, W, Hc, M["stone"], "corps")           # центральный корпус
box(0, 0, 1.2, L*0.47, W+1.0, 2.4, M["plinth"], "plinth")     # цоколь
box(0, 0, Hc+1.2, L*0.46, W, 2.4, M["stone"], "attic")        # аттик
facade_windows(0, W/2+0.2, 0, L*0.9, 3, 4.8, 0.4, 6.0, M["glass"])  # окна фасада
facade_windows(0, -W/2-0.2, 0, L*0.9, 3, 4.8, 0.4, 6.0, M["glass"])
# две башни (Часовая и Сигнальная) — восьмигранные, выше корпуса
for sx in (-L*0.40, L*0.40):
    cyl(sx, 0, Hc*0.62, 9.0, Hc*1.25, M["stone"], "tower", verts=8)
    cyl(sx, 0, Hc*1.25+1.4, 8.5, 2.8, M["plinth"], "tower_top", verts=8)
    cyl(sx, 0, Hc*1.25+4.0, 3.0, 4.0, M["roof"], "flagpole", verts=6, r2=0.2)
# боковые крылья, уходящие назад (к каре)
for sx in (-L*0.40, L*0.40):
    box(sx, -W*1.8, Hc*0.4, 16.0, W*1.4, Hc*0.8, M["stone"], "wing")
export("palace.glb")
