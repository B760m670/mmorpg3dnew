...
H += (fbm(RES, 3, 5, 101) - 0.5) * 16.0    # пологие холмы
H += (fbm(RES, 7, 5, 102) - 0.5) * 7.0     # средние возвышенности и ложбины
H += (fbm(RES, 16, 4, 103) - 0.5) * 3.0    # мелкая волнистость
def _hollow(cx, cz, r, amp):
    return -amp * np.exp(-(((GX-cx)**2 + (GZ-cz)**2) / (r*r)))
for hx, hz, hr, ha in [(-300, 400, 120, 3.0), (500, -200, 150, 2.5), (200, 600, 100, 2.0), (-600, -300, 140, 2.8)]:
    H += _hollow(hx, hz, hr, ha)
