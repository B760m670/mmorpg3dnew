...
SMOOTH_M = 2.5        # м: сглаживание поля под сетку местности (у ног она 2 м)
NO_WATER = -32768
...
def main():
    ...
    # --- СГЛАЖИВАНИЕ ПОД СЕТКУ МЕСТНОСТИ ---
    # Поле метровое, а сетка земли у ног — 2 м. Ступеньку в 1 метр она
    # воспроизвести не может: получается гребёнка отвесных стен (видел своими
    # глазами на берегу Белого озера). Сглаживаем до масштаба, который сетка
    # действительно способна показать. Чаша пруда шириной в десятки метров от
    # этого не страдает — уходят только ступеньки.
    out = ndimage.gaussian_filter(out, sigma=SMOOTH_M, mode="nearest")
    # после сглаживания гарантию у кромки восстанавливаем (сглаживание её сняло)
    if np.isfinite(level_img).any():
        ...
        for k3 in range(1, cnt + 1):
            ...
            if comp3.sum() < MIN_AREA_M2:
                continue
            lv3 = float(np.nanmax(level_img[comp3]))
            collar = ndimage.binary_dilation(comp3, iterations=COLLAR) & land
            out[collar] = np.maximum(out[collar], lv3 + BANK_MIN)
            # дно тоже возвращаем: сглаживание подняло его у берега
            d3 = ndimage.distance_transform_edt(comp3)
            dep3 = np.minimum(d3 * SLOPE, max_depth_for(int(comp3.sum())))
            out[comp3] = np.minimum(out[comp3], (lv3 - dep3)[comp3])
