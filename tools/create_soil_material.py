...
def build_fields():
    ...
    # ЦВЕТ ЗАМЕРЕН по реальному эталону (ambientCG Ground054 голый суглинок,
    # лин-альбедо ~0.30/0.23/0.14) и пересчитан в sRGB. Дерново-подзолистая
    # почва Гатчины — серо-бурая, светлее прежнего «шоколада» (был в 3× темнее).
    fine = fbm(48, 2)
    base_dark = np.array([0.360, 0.300, 0.220])          # влажный гумус в западинах
    base_dry = np.array([0.560, 0.490, 0.380])           # сухой гребень комка
