...
WANTED = [
    ...
    # ИСПРАВЛЕНО ПОСЛЕ ПРОСМОТРА. Сначала я взял Travertine001/004 по названию —
    # оказалось, это ПОЛИРОВАННАЯ травертиновая плита для интерьера. Пудостский
    # камень пористый и выветренный, полировки на фасаде нет и не было.
    # Ближайшее по характеру — выветренные тёсаные блоки: фасад Ринальди как раз
    # читается сложенным из каменных квадров. Точного пудостского туфа в
    # общественном достоянии нет; это приближение, заменить его сможет только
    # съёмка живого фасада.
    ("камень дворца, выветренные квадры",   "Bricks100",           "2K"),
    ("камень дворца, грубая кладка",        "Bricks084",           "2K"),
    ...
    # ИСПРАВЛЕНО: Ground048 я подписал «гравий дорожки», а это кора-мульча (щепа).
    # Настоящая дворцовая аллея — плотный песчано-гравийный слой, тёплый.
    ("гравий дорожки (аллея)",              "Ground062S",          "2K"),
    ("гравий крупный (проезд)",             "Gravel022",           "2K"),
    ("кора-мульча (клумбы)",                "Ground048",           "1K"),
    # --- покров ---
    ("мох",                                 "Moss001",             "2K"),
