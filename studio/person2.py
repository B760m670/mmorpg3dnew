...
def dress(body):
    ...
    # САПОГ СЕЛ ПО СТОПЕ ВМЕСТЕ С ПАЛЬЦАМИ — в кадре это были не сапоги, а
    # чёрные ступни. У обуви нет пальцев: внутри колодка, снаружи гладкий
    # носок. Лечится большим отступом и сильным разглаживанием — ровно тем,
    # что стирает мелкий рельеф и оставляет объём.
    def keep_boot(c):
        return c.z < Y_ANKLE + 0.05
    boots = cut(body, "Сапоги", keep_boot, offset=0.016, relax=14)
    ...
    # КОСОВОРОТКА ВЫЛЕЗЛА ХОМУТОМ поверх пальто: выкройка шла от плеч, а
    # видно из-под воротника должно быть только горло. Оставляем узкую полосу
    # у шеи и по оси — рукавов и плеч у неё в кадре нет.
    def keep_shirt(c):
        return (Y_SHOULDER - 0.05 < c.z < Y_CHIN - 0.015
                and math.hypot(c.x, c.y) < 0.11)
