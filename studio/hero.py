...
FACE_SHAPE = {
    # ПОДБОРОДОК: 0.80 -> 0.35 -> 0.00. Заказчик сказал «подбородок реально
    # большой» уже ПОСЛЕ первой правки — и был прав, а мерить это было нечем:
    # ни одна из четырёх опор не стояла на подбородке. Когда опора нашлась
    # (перегиб профиля, см. measure_face.chin_by_slope), число стало видно:
    # рот-подбородок 58.0 мм при норме 50.6. Цель при этом действует ровно:
    # 0.35 -> 58.0 мм, 0.00 -> 50.5 мм, -0.60 -> 44.5 мм. Ноль и берём.
    "chin-height": 0.00,
    "head-scale-vert": 0.10,    # добирает высоту головы, тянет и ухо
    ...
    # ПОПЕРЕЧНАЯ ПОДНЯТА С 0.10 ДО 0.28, а подбородок сбавлен с 0.80 до 0.35.
    #
    # Заказчик сказал: лицо слишком вытянутое, подбородок длинным кажется.
    # Промеры при этом сходились с ANSUR в пределах 4% — и противоречия тут
    # нет. Промер отвечает на вопрос «сколько миллиметров», а «вытянутое
    # лицо» — это ОТНОШЕНИЕ высоты к ширине, и его в таблице не было вовсе.
    # Добавлен (measure_face.ОТНОШЕНИЯ); ИЗМЕРЕНО: голова длина/ширина была
    # 133.6 при норме 129.3, стала 129.2 — ровно в цель.
    # Заодно сошлись межзрачковое (−2.9% -> +0.5%), высота уха над макушкой
    # (+2.3% -> +0.5%) и ширина головы (−1.7% -> +1.8%).
    #
    # ЧЕМ ЗАПЛАТИЛИ: рот стал шире, +11.2% -> +15.1%. Возвращаем его целью
    # ширины рта — она про рот и ни про что другое.
    "head-scale-horiz": 0.28,
    "mouth-scale-horiz": -0.45,
}
...
WEAR = [
    ...
    ("Сапоги", "clothes/rehmanpolanski_viking_boots/rehmanpolanski_viking_boots.mhclo",
     0.003, (0.045, 0.040, 0.038)),
    ("Пальто", "clothes/mindfront_cardigan_long_open_front/"
               "mindfront_cardigan_long_open_front.mhclo",
     # СВЕТЛЕЕ, ЧЕМ БЫЛО (0.17), И ЭТО РЕШЕНО ЗАМЕРОМ, А НЕ ВКУСОМ.
     # Пальто и сапоги сливались в одно пятно: яркость в кадре 0.144 против
     # 0.116, отношение 1.24. Сапог ушёл в ваксу (0.045) и получил блеск,
     # пальто поднялось до 0.24 — отношение стало 1.96, силуэт разделился.
     0.018, (0.24, 0.225, 0.205)),
...
def wardrobe(body):
    ...
    for name, rel, off, rgb in WEAR:
        ...
        for o in new:
            ...
            # БЛЕСК РАЗНЫЙ У РАЗНЫХ ТКАНЕЙ, И ЭТО НЕ УКРАШЕНИЕ.
            # ИЗМЕРЕНО в кадре игры: яркость пальто 0.144, сапога 0.116 —
            # отношение 1.24. По такой разнице силуэт не читается, пальто и
            # сапоги слились в одно пятно, что заказчик и увидел. Я сам к
            # этому и привёл, сняв блик со ВСЕЙ одежды разом: у сукна его
            # действительно нет, а вычищенный ваксой сапог блестит — это
            # главное, чем кожа отличается от шерсти, и глазу этого хватает
            # даже когда тона близки.
            cloth.matte(o, *((0.38, 0.55) if name == "Сапоги" else (0.92, 0.18)))
            cloth.flat(o)
            if name == "Косоворотка":
                cloth.unbind(o, ("LeftUpLeg", "RightUpLeg"))
