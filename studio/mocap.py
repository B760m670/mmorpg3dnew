...
def load_cmu(arm, asf_path, amc_path, start=1, count=0, fps=120, step=1,
    ...
    # в ключ кости.
    # ЕСЛИ В ЗАПИСИ КОСТЬ НЕ ДВИГАЕТСЯ, ПЕРЕНОС НЕ ИМЕЕТ ПРАВА ЕЁ ДОВОРАЧИВАТЬ.
    #
    # Выравнивание базы, описанное выше, делает вот что: заставляет НАШУ кость
    # смотреть туда же, куда смотрит кость CMU. Для бедра это и нужно. Но оно
    # вносит ПОСТОЯННЫЙ доворот КАЖДОЙ кости, даже той, у которой в записи
    # ровно нули. Посчитано для 07_01.amc (доворот при нулевом движении):
    #   бедро      57-60°     предплечье  50.7°
    #   тазобедр.  43-46°     шея         45°, голова 36.5°
    #   КЛЮЧИЦА    34.4°      плечо       22.8-24.0°
    # А ключицы в этой записи неподвижны — размах по обоим каналам ровно 0.00°
    # за все 316 кадров. То есть 34° им добавил перенос из ничего.
    #
    # В кадре это и было видно заказчику: ИЗМЕРЕНО, плечевой сустав
    # относительно основания шеи в покое −103.9 мм, а в шаге −10.5..−29.7 мм,
    # то есть плечи вздёрнуты на 79-87 мм. Человек при ходьбе плечи не
    # поднимает вовсе. И одежда тут ни при чём — то же самое на голом теле.
    #
    # ПРАВИЛО: у кости, чьи каналы за всю запись не отходят от нуля, база не
    # своя, а РОДИТЕЛЬСКАЯ. Тогда постоянный доворот выходит тождественным
    # (проверяется алгеброй: ключ = Rb^T Rp base_p^T base_b, и при
    # base_b = align_p·Rb он равен единице), а движение родителя кость
    # по-прежнему наследует — ключица так и должна ездить за грудным отделом.
    #
    # Просто выключить выравнивание нельзя, это проверено: без него походка
    # рассыпается совсем — «постановок левой стопы меньше двух, цикл не
    # выделить», и таз приходится опускать на 76 мм.
    # МЕРА НЕПОДВИЖНОСТИ — РАЗМАХ, А НЕ НУЛЬ. Первый заход ловил кости, у
    # которых все значения ровно нулевые, и пропустил пальцы: у них размах
    # ровно 0.00°, но значение 7.13° — постоянный лёгкий подгиб. Правило их не
    # тронуло, и пальцы получили от выравнивания 21° из ничего: в кадре кисть
    # висела доской с растопыренными пальцами.
    # Постоянное ненулевое значение при этом СОХРАНЯЕТСЯ: оно приходит из
    # записи и применяется поверх родительской базы, как и должно.
    неподвижные = set()
    for b in bones:
        столбцы = [fr.get(b, []) for fr in frames]
        n = max((len(c) for c in столбцы), default=0)
        if n == 0:
            неподвижные.add(b)
            continue
        размах = max(max(c[i] for c in столбцы if len(c) > i)
                     - min(c[i] for c in столбцы if len(c) > i)
                     for i in range(n))
        if размах < 1e-9:
            неподвижные.add(b)
    # ШЕЙНЫЙ ОТДЕЛ И ГОЛОВА — ТОЖЕ ПО РОДИТЕЛЬСКОЙ БАЗЕ, И ЭТО ЗАМЕРЕНО.
    # Заказчик увидел запрокинутую голову. ИЗМЕРЕНО: взгляд (ось −Z кости
    # головы) при ходьбе шёл на +4.8..+13.3° ВЫШЕ горизонта, средн +11.0°, —
    # человек так не ходит. В записи голова наклонена ВПЕРЁД (канал head rx
    # в среднем 12.87°), значит запрокидывало её выравнивание базы: шее оно
    # добавляет 45° постоянного доворота, голове 36.5°.
    # С родительской базой взгляд встаёт на −11.2..−2.6°, средн −5.0° — это
    # взгляд на дорогу метрах в десяти, ровно человеческий.
    # ЦЕНА: никакой. Цикл 1.423 м за 1.100 с, опорная подошва −0.1 мм от пола,
    # снос 0.0 мм — всё до последней цифры как было.
    # СТОПЫ СЮДА НЕ ВХОДЯТ, И ЭТО ПРОВЕРЕНО: с ними носок разворачивается
    # внутрь ещё сильнее (−63.7° против −51.0°), а подошва тонет на 24.7 мм
    # вместо 14.2. Правило верное не «везде», а там, где показал замер.
    ШЕЯ = ("lowerneck", "upperneck", "head")
    неподвижные |= set(родительская) | set(ШЕЯ)
    if неподвижные:
        print("[захват] база родительская у: %s"
              % ", ".join(sorted(неподвижные)))

    rest = {}      # наша поза покоя: в ней записаны ключи
    base = {}      # выровненная под CMU: в ней считается мировая ориентация
    for asf_n in order:
        bl_n = BONES.get(asf_n)
        bb = arm.data.bones.get(bl_n) if bl_n else None
        if not bb:
            continue
        ...
        p = par.get(asf_n)
        if asf_n in неподвижные and p in rest:
            # родительский доворот, приложенный к НАШЕЙ кости покоя
            align_p = base[p] @ rest[p].transposed()
            base[asf_n] = align_p @ R
        elif align and d.length > 1e-6:
