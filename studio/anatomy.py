...
# живых мужчин.
#
# ЧЕГО Я НЕ УВИДЕЛ И ЧТО УВИДЕЛ ЗАКАЗЧИК: «грудь слишком большая, как у
# девушки, соски должны быть выше, соски втянуты внутрь». Всё три — правда, и
# всё три меряются. Что показал замер:
#   1. СОСОК — ЭТО КРАСКА, а не геометрия: он нарисован на фотоскане кожи, и
#      где он окажется, решает развёртка. Найден по цвету на самой текстуре
#      (тексель 665 и 897 из 2048) и переведён через развёртку на тело:
#      высота 0.7285 роста при норме ANSUR 0.7352 — НИЖЕ НА 12 ММ.
#   2. СОСОК ВТЯНУТ, и это не обман зрения: центр стоит на 2.3 мм ПОЗАДИ
#      кольца в 22 мм вокруг, а у ста живых мужчин сосок ВЫСТУПАЕТ на
#      5.0 ± 1.1 мм. Промах 7.3 мм. У базовой сетки MakeHuman на мужской
#      груди на этом месте воронка, а не сосок.
#   3. ГРУДЬ ВЕЛИКА ЧИСЛОМ, А НЕ НА ГЛАЗ: обхват груди 1061 мм без рельефа и
#      1091 с ним, при нашей цели 1004. То есть рельеф добавил 30 мм обхвата
#      сверх и без того превышенного. Правильный рельеф ничего не добавляет:
#      он ПЕРЕКЛАДЫВАЕТ объём — вперёд у брюшка, назад у грудины.
#   4. ПОЧЕМУ ЧИТАЛОСЬ ЖЕНСКИМ. Брюшко мышцы по высотной привязке приходится
#      на 104 мм от средней линии, а сосок нарисован на 91. Сосок сидел на
#      ВНУТРЕННЕМ СКЛОНЕ массы, а не на её гребне; у женской груди сосок
#      именно так и лежит. Разводится с двух сторон: краску сдвигаем наружу
#      правкой развёртки (форма тела при этом не трогается вовсе), мышцу —
#      внутрь отдельным поперечным масштабом.
#
# ПОЧЕМУ МАСШТАБ ПОПЕРЁК И ВДОЛЬ РАЗНЫЙ. Вдоль он проверяем: верх мышцы
# обязан попасть на яремную вырезку, и при L=180 мм выходит 0.807 против
# 0.813 у ANSUR. Поперёк такой проверки нет, зато есть сосок, к которому
# брюшко и привязывается. Числа поэтому разные, и это не подгонка под глаз.
# ИЗМЕРЕНО перебором: при 1.00 промах по поперечному разрезу до 4.1 мм, при
# 0.70 — до 3.0. Больше не значит вернее: после того как сосок и брюшко свели
# вместе, полная недостача стала перебором.
PEC_GAIN = 0.70      # доля измеренной недостачи; ставится по обхвату груди
PEC_FURROW = 0.008   # ложбина по грудине, назад; ставится по обхвату груди
...
STERNUM_L = 0.180    # длина грудины на нашем теле, проверена яремной вырезкой

# --- сосок -------------------------------------------------------------------

NIP_KEY = "анатомия: соски"
# Ареола на фотоскане, найдена по цвету: тексели (665, 617) и (897, 617) из
# 2048. Развёртка Блендера считает снизу, отсюда 1 − y/2048.
NIP_UV = ((665.0 / 2048.0, 1.0 - 617.0 / 2048.0),
          (897.0 / 2048.0, 1.0 - 617.0 / 2048.0))
# ИЗМЕРЕНО у ста мужчин: сосок выступает на 5.0 ± 1.1 мм — но он и ШИРИНОЙ
# 5 мм, а ареола вокруг него плоская. У нашей сетки на груди вершины стоят
# через 6 мм (152 вершины на 5310 мм² правки), то есть сосок в 5 мм — МЕЛЬЧЕ
# ОДНОЙ ЯЧЕЙКИ и геометрией не выражается вовсе. Первая попытка поставить
# полные 5 мм на ареолу целиком дала в кадре не сосок, а заклёпку.
# Поэтому геометрией поднимается только мягкий бугор ареолы, а сам сосок
# остаётся там, где ему и место при такой сетке — в фотоскане кожи.
NIP_STAND_MM = 2.5   # мягкий подъём ареолы, а не сосок
NIP_R = 0.016        # радиус ареолы: 25 мм поперёк, те же сто мужчин
NIP_CRATER_R = 0.026  # радиус воронки базовой сетки, которую надо засыпать
NIP_MOVE_OUT = 0.011  # НАПОЛОВИНУ: краска идёт наружу на 11 мм, мышца внутрь
                      # на столько же. Целиком двигать краску (21 мм) — сильно
                      # тянуть кожу, целиком мышцу — ломать высотную сверку.
NIP_WARP_R = 0.075    # радиус правки развёртки, в единицах UV


def _uv_verts(body, uv, rad=0.008):
    """Вершины, на которые развёртка кладёт кусок текстуры вокруг uv."""
    me = body.data
    ...
    for poly in me.polygons:
        for li in poly.loop_indices:
            a, b = lay.data[li].uv
            d = ((a - uv[0]) ** 2 + (b - uv[1]) ** 2) ** 0.5
            if d < rad:
                vi = me.loops[li].vertex_index
                hit[vi] = min(hit.get(vi, 9e9), d)
    return [i for i, _ in sorted(hit.items(), key=lambda kv: kv[1])]


def nipple_pos(body, n=12, uvs=None):
    """Где НА ТЕЛЕ оказывается нарисованный сосок, по стороне. В мире.

    uvs — где краска лежит СЕЙЧАС. После переноса пятна в картинке это уже не
    NIP_UV, и проверять надо по новым местам, иначе померишь пустую кожу.
    """
    cur, _ = _base_positions(body)
    M = body.matrix_world
    out = []
    for uv in (uvs if uvs is not None else NIP_UV):
        idx = _uv_verts(body, uv)[:n]
        pts = [M @ cur[i] for i in idx if i < len(cur)]
        out.append(sum(pts, Vector()) / len(pts) if pts else None)
    return out


def _uv_at(body, target):
    """Развёртка в точке тела, ближайшей к target (только передняя сторона)."""
    me = body.data
    lay = me.uv_layers.active
    cur, real = _base_positions(body)
    M = body.matrix_world
    ...
    for poly in me.polygons:
        for li in poly.loop_indices:
            ...
            d = (p - target).length
            if d < bd:
                bd, best = d, Vector(lay.data[li].uv)
    return best, bd


def nipples(body, verbose=True):
    """Поставить сосок туда, где он у человека, и заставить его ВЫСТУПАТЬ.

    ДВЕ РАЗНЫЕ ПРАВКИ, И ПУТАТЬ ИХ НЕЛЬЗЯ.
    Сосок нарисован на коже, поэтому по телу он двигается ПРАВКОЙ РАЗВЁРТКИ:
    форма тела при этом не меняется ни на миллиметр, съезжает только рисунок.
    А вот втянутость — это геометрия: у базовой сетки MakeHuman на этом месте
    воронка глубиной около 6 мм. Её засыпаем и поверх поднимаем сосок ключом
    формы. Если сделать наоборот — двигать сосок сдвигом вершин, — поедет вся
    грудь, а краска останется на месте.
    """
    me = body.data
    lay = me.uv_layers.active
    if lay is None:
        if verbose:
            print("[сосок] у сетки нет развёртки — правка невозможна")
        return 0
    cur, real = _base_positions(body)
    nrm = _normals(body)
    M = body.matrix_world
    ...
    if any(p is None for p in now):
        if verbose:
            print("[сосок] не нашёлся по развёртке")
        return 0

    want, uv_target = [], []
    for side, p in enumerate(now):
        sgn = 1.0 if p.x >= 0 else -1.0
        tgt = Vector((sgn * (abs(p.x) + NIP_MOVE_OUT * k),
                      p.y, lo + NIPPLE_Z * H))
        uvt, d = _uv_at(body, tgt)
        want.append(tgt)
        uv_target.append(uvt)
        if verbose:
            print("[сосок] %-6s было: %.4f роста, %.0f мм от середины;  "
                  "надо: %.4f, %.0f мм"
                  % ("левый" if side == 0 else "правый",
                     (p.z - lo) / H, abs(p.x) * 1000,
                     (tgt.z - lo) / H, abs(tgt.x) * 1000))

    # 1) КРАСКА ДВИГАЕТСЯ В САМОЙ КАРТИНКЕ, а не правкой развёртки.
    # Правку развёртки я сделал первой и выбросил: кадр с разделением причин
    # (только развёртка, ключи формы выключены) показал, что ареола
    # превращается в размазанный завиток. Подробности — в studio/skin_edit.py.
    import skin_edit
    warped = 0
    img = skin_edit.find_skin_image(body)
    if img is None:
        if verbose:
            print("[сосок] картинки кожи в материале нет — краску не двигаем")
    else:
        W, Ht = img.size
        moves = []
        for side, uvt in enumerate(uv_target):
            if uvt is None:
                continue
            s = (NIP_UV[side][0] * W, (1.0 - NIP_UV[side][1]) * Ht)
            d = (uvt[0] * W, (1.0 - uvt[1]) * Ht)
            moves.append(((int(round(s[0])), int(round(s[1]))),
                          (int(round(d[0])), int(round(d[1])))))
        if moves:
            warped = skin_edit.move_spot(img, moves, verbose=False)

    # 2) ГЕОМЕТРИЯ: засыпать воронку на старом месте, поднять сосок на новом
    if me.shape_keys is None:
        body.shape_key_add(name="Basis", from_mix=False)
    key = me.shape_keys.key_blocks.get(NIP_KEY)
    if key is None:
        key = body.shape_key_add(name=NIP_KEY, from_mix=False)
    key.value = 1.0
    basis = me.shape_keys.key_blocks[0]

    # глубина воронки меряется, а не назначается
    fill = []
    for p in now:
        ring = [(M @ q).y for q, r in zip(cur, real) if r
                and 0.018 < ((M @ q) - p).length < 0.030 and (M @ q).y < 0]
        fill.append(max(0.0, p.y - (sum(ring) / len(ring))) if ring else 0.0)
    ...
    for i, co in enumerate(cur):
        ...
        amp = 0.0
        for side in range(2):
            dc = (p - now[side]).length / NIP_CRATER_R
            if dc < 1.0:
                amp += fill[side] * (1.0 - dc * dc) ** 2
            dn = (p - want[side]).length / NIP_R
            if dn < 1.0:
                amp += (NIP_STAND_MM / 1000.0 * k) * (1.0 - dn * dn) ** 2
        if abs(amp) < 1e-5:
            continue
        n = nrm[i] if i < len(nrm) else Vector((0.0, -1.0, 0.0))
        if n.length < 1e-6:
            n = Vector((0.0, -1.0, 0.0))
        key.data[i].co = basis.data[i].co + (Mi.to_3x3() @ (n.normalized() * amp))
        moved += 1
        mx = max(mx, amp)
    bpy.context.view_layer.update()
    if verbose:
        print("[сосок] краска перенесена в картинке (%d пятна); воронка базовой "
              "сетки засыпана на %.1f/%.1f мм; сдвинуто %d вершин, до %.1f мм"
              % (warped, fill[0] * 1000, fill[1] * 1000, moved, mx * 1000))
        got = nipple_pos(body, uvs=[u for u in uv_target if u is not None])
        for side, p in enumerate(got):
            if p is None:
                continue
            print("[сосок] стало: %.4f роста (норма %.4f), %.0f мм от середины"
                  % ((p.z - lo) / H, NIPPLE_Z, abs(p.x) * 1000))
    # КУДА УЕХАЛА КРАСКА — запомнить на теле. Мышца привязывается к соску, а
    # искать его по старой развёртке после переноса нельзя: там уже чистая кожа.
    body["сосок uv"] = [list(u) for u in uv_target if u is not None]
    return moved
...
def pectorals(body, verbose=True):
    ...
    k = H / REF_STATURE
    nip_z = lo + NIPPLE_Z * H
    z_fold = nip_z - 0.033 * k
    # ВДОЛЬ: единица проверена яремной вырезкой (0.807 против 0.813 у ANSUR).
    Lz = STERNUM_L * (H / 1.731)
    # ПОПЕРЁК: брюшко привязано к соску, который сдвинут наружу лишь наполовину
    # (см. NIP_MOVE_OUT). Мышца идёт ему навстречу — отсюда отдельный масштаб.
    # СОСОК БЕРЁТСЯ ТАМ, ГДЕ КРАСКА ЛЕЖИТ СЕЙЧАС. Если nipples() уже отработал,
    # она перенесена, и старая развёртка показывает пустую кожу: мышца тогда
    # садилась на 92 мм вместо 100 и снова уползала внутрь от соска.
    uvs = body.get("сосок uv")
    nx = nipple_pos(body, uvs=[tuple(u) for u in uvs] if uvs else None)
    if all(p is not None for p in nx) and nx:
        nip_x = sum(abs(p.x) for p in nx) / len(nx)
    else:
        nip_x = NIPPLE_X_MM / 1000.0 * k
    # брюшко мышцы лежит на 9 мм ВНУТРЬ от соска (атлас: 105 против 114)
    belly_x = nip_x - 0.009 * k
    Lx = belly_x / PEC_BELLY_U

    def uv(p):
        return abs(p.x) / Lx, (p.z - z_fold) / Lz + PEC_FOLD_V
    ...
    for i, co in enumerate(cur):
        ...
        # ЛОЖБИНА ПО ГРУДИНЕ. Без неё рельеф только ПРИБАВЛЯЕТ объём: обхват
        # груди вырос на 30 мм сверх и без того превышенного. У человека
        # грудина утоплена между мышцами, и правильная правка перекладывает
        # объём, а не досыпает его.
        fx = abs(p.x) / (0.030 * k)
        if fx < 1.0 and t > 0.0:
            amp -= PEC_FURROW * ((1.0 - fx * fx) ** 2) * (t / PEC_T_REF)
        if abs(amp) < 1e-5:
            continue
        n = nrm[i] if i < len(nrm) else Vector((0.0, -1.0, 0.0))
        if n.length < 1e-6:
    ...
    bpy.context.view_layer.update()
    if verbose:
        print("[грудные] единица вдоль %.0f мм, поперёк %.0f мм; брюшко %.0f мм "
              "от середины (сосок нарисован на %.0f), складка %.3f"
              % (Lz * 1000, Lx * 1000, belly_x * 1000, nip_x * 1000, z_fold))
        print("[грудные] сдвинуто %d вершин, наружу до %.1f мм, внутрь до %.1f мм; "
              "верх мышцы на %.3f роста (яремная вырезка у ANSUR 0.813)"
              % (moved, mx * 1000, -mn * 1000,
                 ((z_fold + (0.02 - PEC_FOLD_V) * Lz) - lo) / H))
    return moved
