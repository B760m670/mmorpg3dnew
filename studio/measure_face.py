...
def chin_z(body):
    """Высота подбородка: под ним профиль резко уходит НАЗАД, в шею.

    Идём сверху вниз по средней линии. Нос, губы, подбородок — всё это
    выступы; сразу под подбородком поверхность отступает назад быстрее, чем
    где бы то ни было ещё. Берём самый большой такой отступ.
    """
